% [sumEnergy] = mod_reconstructEnergyFromAtomsMPP(gaborData,N,wrap,atomList)
...
function [sumEnergy] = mod_reconstructEnergyFromAtomsMPP(gaborData,N,wrap,atomList)
...
halfN = floor(N/2); % TG: check this

sumEnergy = zeros(halfN+1,N);
maxOct = nextpow2(N);
...
for i=1:length(atomList)
    ...
    if oct==0                       % Dirac
        ...
        % TG: check this
        %sumEnergy(2:halfN,u+1) = mod2*(2/N) + sumEnergy(2:halfN,u+1);
        %sumEnergy(1,u+1)     = mod2*(1/N) + sumEnergy(1,u+1);
        %sumEnergy(halfN+1,u+1) = mod2*(1/N) + sumEnergy(halfN+1,u+1);
        sumEnergy(2:halfN,min(u+1,N)) = mod2*(2/N) + sumEnergy(2:halfN,min(u+1,N));
        sumEnergy(1,min(u+1,N))     = mod2*(1/N) + sumEnergy(1,min(u+1,N));
        sumEnergy(halfN+1,min(u+1,N)) = mod2*(1/N) + sumEnergy(halfN+1,min(u+1,N));
        
    elseif oct==maxOct              % Fourier
        
        % TG: check this
        % sumEnergy(ksi+1,:) = mod2*(1/N) + sumEnergy(ksi+1,:);
        sumEnergy(min(ksi,halfN)+1,:) = mod2*(1/N) + sumEnergy(min(ksi,halfN)+1,:);
        ...
        if oct==maxOct-1 % range of +-N
            range = [0 N-1];
            if wrap
                [E0,f] = mod_wignerVille(oct,u,ksi,nFFT,range,limitFreq);
                E1 = mod_wignerVille(oct,u-N,ksi,nFFT,range,limitFreq);
                E2 = mod_wignerVille(oct,u+N,ksi,nFFT,range,limitFreq);
                E = (E0+E1+E2);
                normE = sum(sum(E));
                sumEnergy(f+1,:) = mod2*E/normE + sumEnergy(f+1,:);
            else
                [E,f,normE2] = mod_wignerVille(oct,u,ksi,nFFT,range,limitFreq);
            ...
            if wrap
                if (u < s2)
                    ...
                    [E0,f,normE20] = mod_wignerVille(oct,u,ksi,nFFT,r0,limitFreq);
                    r1 = [N-1-(s2-u) N-1];
                    [E1,~,normE21] = mod_wignerVille(oct,u+N,ksi,nFFT,r1,limitFreq);
                ...
                elseif (u > N-1-s2)
                    ...
                    r = [u-s2 u+s2];
                    [E,f,normE2] = mod_wignerVille(oct,u,ksi,nFFT,r,limitFreq);
