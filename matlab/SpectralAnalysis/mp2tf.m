...
% Modified to use ceiling
t=1:ceil(maxT-minT); % skala czasu w punktach
f=1:ceil(maxF-minF); % skala czestosci w punktach
Tsize=length(t);
Fsize=length(f);
Xsize=ceil((maxT-minT)/Dt); % Fix
Ysize=ceil((maxF-minF)/Df); % Fix
